...
export const SYH_PARSERS = {
  ...
  parseEmojiNumberedQuestions: function(rawText) {
    ...
    const MAX_LENGTH = SYH_CONFIG && SYH_CONFIG.LIMITS && SYH_CONFIG.LIMITS.TEXT_TRUNCATION_LENGTH || typeof window !== "undefined" && window.SYH_CONFIG && window.SYH_CONFIG.LIMITS && window.SYH_CONFIG.LIMITS.TEXT_TRUNCATION_LENGTH || 195;
    const ELLIPSIS = "...";
    const truncate = (text) => {
...
${newBody}`;
  ...
  parseStandardNumberedQuestions: function(rawText) {
    ...
    const maxLen = typeof SYH_CONFIG !== "undefined" && SYH_CONFIG && SYH_CONFIG.LIMITS && SYH_CONFIG.LIMITS.TEXT_TRUNCATION_LENGTH || typeof window !== "undefined" && window.SYH_CONFIG && window.SYH_CONFIG.LIMITS && window.SYH_CONFIG.LIMITS.TEXT_TRUNCATION_LENGTH || 195;
    return formattedText.split("\n").map((line) => line.trim()).filter((line) => /^\d+\./.test(line)).map((line) => line.replace(/^\d+[.)]?\s*/, "").replace(/\s*\([^)]+\)$/, "").trim()).filter((line) => line.length > 0).map((line) => {
      if (line.length >= 200) {
  ...
  parseSabbathSchoolUnnumberedQuestions: function(rawText) {
    ...
    const maxLen = SYH_CONFIG && SYH_CONFIG.LIMITS && SYH_CONFIG.LIMITS.TEXT_TRUNCATION_LENGTH || typeof window !== "undefined" && window.SYH_CONFIG && window.SYH_CONFIG.LIMITS && window.SYH_CONFIG.LIMITS.TEXT_TRUNCATION_LENGTH || 195;

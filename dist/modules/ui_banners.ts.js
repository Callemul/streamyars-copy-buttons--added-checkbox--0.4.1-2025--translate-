...
export function addButtonsToBanner(bannerNode) {
...
export function updateBannerVisuals($bannerBlock, type) {
...
export function applySavedBannerLabels(bannerNode, text) {
  if (!text || !text.trim()) return;
  const type = SYH_UI.bannerCategoriesCache[text] || "none";
  updateBannerVisuals($(bannerNode), type);
}
export function addBannerHeaderControls(headerNode) {
...
export function filterBanners() {
  ...
  $bannerList.find('> li, > div[class*="Banner__LiWrap"]').each(function() {
    ...
    if (searchQuery) {
      matchesSearch = SYH_UTILS && typeof SYH_UTILS.smartSearch === "function" ? SYH_UTILS.smartSearch(searchQuery, originalText) : window.SYH_UTILS && typeof window.SYH_UTILS.smartSearch === "function" ? window.SYH_UTILS.smartSearch(searchQuery, originalText) : originalText.toLowerCase().includes(searchQuery.toLowerCase());
    }
    if (matchesSearch) {
      countSearch.all++;
...
export function scrollToActiveBanner() {
  const bannerListSelector = '[class*="BannerList__ListWrap"], ul[class*="Banner"]';
  const $bannerList = $(bannerListSelector);

export const SYH_CONFIG = {
  SELECTORS: {
    ...
    get timerOffTextResult() {
      return typeof chrome !== "undefined" && chrome.i18n && chrome.i18n.getMessage("timerOff") || "Timer off";
    },
    // НОВІ СЕЛЕКТОРИ ДЛЯ НАГАДУВАННЯ
    streamStatusContainer: '[class*="Tags__Wrap"]',
    // Контейнер, де з'являється "Ended"
    reminderTargetContainer: '[data-testid="header-title-wrap"]'
    // Куди вставляти нагадування
  },
  TIMINGS: {
    ANTI_AFK_INTERVAL: 3e4,
    // 30 секунд — інтервал Anti-AFK кліків
    AUTO_HEAL_POLLING: 500,
    // 500мс — DOM polling для Auto-Heal
    FILTER_DEBOUNCE: 150,
    // 150мс — debounce для фільтру пошуку
    STATS_TRACKING_INTERVAL: 6e4,
    // 60 секунд — інтервал збору статистики
    REMINDER_DELAY: 2 * 60 * 1e3
    // 2 хвилини — затримка нагадування
  },
  LIMITS: {
    TEXT_TRUNCATION_LENGTH: 195
    // Максимальна довжина тексту перед обрізанням
  }
};
if (typeof window !== "undefined") {
  window.SYH_CONFIG = SYH_CONFIG;
}

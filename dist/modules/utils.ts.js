...
export const SYH_UTILS = {
  ...
  get storage() {
    return SYH_STORAGE || (typeof window !== "undefined" ? window.SYH_STORAGE : void 0);
  },
  ...
  waitForElement: function(selector, timeout = 3e3) {
  ...
  waitForElementToDisappear: function(selector, timeout = 3e3) {
    return new Promise((resolve, reject) => {
      const interval = 100;
      let elapsedTime = 0;
      const timer = setInterval(() => {
  ...
  waitForNewBanner: function(bannerText, timeout = 5e3) {
    return new Promise((resolve, reject) => {
      const interval = 100;
      let elapsedTime = 0;
      const timer = setInterval(() => {
  ...
  clickElementByText: function(text, timeout = 2e3) {
  ...
  smartSearch: function(query, targetText) {
    ...
    return queryWords.every((word) => {
      ...
      return fullTarget.includes(normWord) || fullTarget.includes(transWord) || fullTarget.includes(layoutWord) || fuzzyWord && fullTarget.includes(fuzzyWord);
    });
  },
  ...
  toFuzzy: function(str) {
    ...
    for (let i = 0; i < s.length; i++) {
      const char = s[i];
      res += singleMap[char] !== void 0 ? singleMap[char] : char;
    }
    return res;
  },

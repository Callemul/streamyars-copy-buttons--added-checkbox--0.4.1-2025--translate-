...
## 🔢 `noUncheckedIndexedAccess` увімкнено — індекс не гарантує значення

`arr[i]`, `str[i]`, `map[key]` мають тип `T | undefined`. Це навмисно: індекс
у межах масиву — це інваріант коду, а не факт, відомий компілятору.

Як писати, щоб не боротися з прапорцем:

| Замість | Пишіть |
|---|---|
| `for (let i = 0; i < arr.length; i++) arr[i]` | `for (const item of arr)` або `arr.entries()` |
| `str[i]` | `str.charAt(i)` — тип `string`, поведінка та сама |
| `Object.keys(o)` + `o[k]` | `Object.entries(o)` / `Object.values(o)` |
| `if (changes[KEY]) changes[KEY].newValue` | підняти `changes[KEY]` в локальну змінну |
| `Record<string, number>` з фіксованим набором ключів | іменований тип (`CommentTabCounts`) |

**`!` і `as` — не рішення.** Вони прибирають повідомлення, не додаючи
безпеки. Якщо індекс справді в межах за інваріантом коду — пишіть
`?? <нейтральне значення>` і **коментар, чому фолбек недосяжний**. Тоді
наступний читач бачить інваріант, а не просто мовчазне ствердження.

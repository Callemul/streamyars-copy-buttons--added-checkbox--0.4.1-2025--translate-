...
> **Статус:** ✅ ВИПРАВЛЕНО (до 2026-08-10). `event_comments/types.ts` містить усі необхідні `import type`, а `config.ts` надає `toSelectorList`/`closestBySelectorValue`; `tsc --noEmit` чистий (0 помилок). Перевірено `tests/event_comments*.test.js`.

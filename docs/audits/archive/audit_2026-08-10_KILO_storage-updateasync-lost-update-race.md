...
> **Статус:** ✅ ВИПРАВЛЕНО (2026-08-10). `storageUpdateAsync` серіалізовано через чергу на ключ (`updateQueues`) і пише лише змінені ключі (без write-amplification). Усунено втрату оновлень у межах JS-контексту. Додано регрес-тести в `tests/storage_adapter.test.js`.
---

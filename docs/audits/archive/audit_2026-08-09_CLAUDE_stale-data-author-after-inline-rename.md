...
> **Статус:** ✅ ВИПРАВЛЕНО (2026-08-10). `saveAuthorRename`/`savePrayerTextEdit` викликають `renderPrayers(list)` після зміни, тож DOM (зокрема `data-author` на кнопках видалення) синхронізується зі сховищем. Додано регрес-тест у `tests/prayer_handlers.test.js`.

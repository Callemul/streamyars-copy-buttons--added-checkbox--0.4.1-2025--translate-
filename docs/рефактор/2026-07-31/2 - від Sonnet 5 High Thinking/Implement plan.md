...
## 0. БЛОКЕР — потрібно від Олександра перед білдом

Старий модуль (`youtube/*`) має працювати лише на 2 каналах. Матчимо по `href="/@handle"`
елемента `ytd-channel-name#channel-name #text-container a[href^="/@"]` (взято зі скріну).

**Потрібні handle'и (`/@...`) двох каналів:**
- [ ] "Время перемен" → `@______`
- [ ] "Слово живое" → `@______`

Без них модуль піде в білд з `handles: ['@TODO_...']` — працювати не буде на жодному каналі
(safe default = нікого не пускати), щоб не зламати продакшн раптово.
...
| # | Рішення |
|---|---|
| A | Вкладка "Telegram Питання 🚀" → **"Telegram/YouTube Питання 🚀"**. Всередині — 4 під-вкладки (pill-nav, той самий стиль що й `.tabs`), кожна = повний клон поточного макету Telegram-вкладки. Візуал не змінюється, лише розмноження + namespacing storage-ключів. |
